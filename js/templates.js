...
function createProductCardHTML(product) {
    ...
    const imageUrl =
        (product.images && product.images.length > 0
            ? product.images[0]
            : product.image) || IMAGE_PLACEHOLDER_PRODUCT;
    ...
    <div class="product-card" data-product-id="${product.id}">
      ...
      <div class="product-card__content">
        ...
        <div class="product-card__bottom-actions">
          <button class="product-card__icon-btn add-to-wishlist" data-product-id="${
        product.id
    }" aria-label="Добавить в избранное">
            <img src="${ASSET_HEART_SVG}" alt="В избранное">
          </button>
          <button class="product-card__icon-btn add-to-cart-btn" data-product-id="${
        product.id
