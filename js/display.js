...
(function () {
        ...
        try {
            ...
            if (categories && categories.length > 0) {
                const featured = categories.filter(
                    (cat) => categories.featured === true || categories.featured === 1
                );

                const toDisplay =
                    featured.length > 0 ? featured : categories.slice(0, 4); // Показать 4, если нет featured

                container.innerHTML = toDisplay
                    .map(window.createCategoryCardHTML)
                    .join("");

                if (typeof window.createCategoryCardHTML !== "function") {
                    console.error("window.createCategoryCardHTML is not a function!");
                    container.innerHTML =
                        "<p class='error-message'>Ошибка: шаблон для категорий не найден.</p>";
                    return;
                }
                ...
                if (toDisplay.length > 0) {
                    ...
                    console.warn(
                        '[displayHomepageCategories] "toDisplay" is empty, showing "Рекомендуемые категории не найдены."'
                    );
                    container.innerHTML = "<p>Рекомендуемые категории не найдены.</p>";
                }
            } else {
                console.warn(
                    '[displayHomepageCategories] "categories" is empty or not an array, showing "Категории не найдены."'
                );
                container.innerHTML = "<p>Категории не найдены.</p>";
            }
        } catch (e) {
            container.innerHTML =
                '<p class="error-message">Ошибка загрузки категорий.</p>';
        ...
        try {
            ...
            if (categories && categories.length > 0) {
                container.innerHTML = categories
                    .map(
                        (categories) =>
                            `<li><a href="catalog.html?category=${
        ...
        try {
            ...
            if (popular && popular.length > 0) {
                container.innerHTML = popular
                    ...
                    .join("");
                if (window.Search && Search.setupPopularSearch)
                    Search.setupPopularSearch(popular);
                if (window.Search && Search.setupProductCardActionsForContainer)
                    Search.setupProductCardActionsForContainer(container);
            } else {
                container.innerHTML = "<p>Популярные товары не найдены.</p>";
            }
        } catch (e) {
            container.innerHTML =
                '<p class="error-message">Ошибка загрузки популярных товаров.</p>';
            console.error(e);
        }
    }
            ...
            if (reviews && reviews.length > 0) {
              ...
              <div class="review-card swiper-slide">
                <div class="review-card__author">
                    <img src="${
                            review.avatar || window.IMAGE_PLACEHOLDERS.AVATAR_DEFAULT
                        }" alt="${
                    ...
                    <div>
                      ...
                      <div class="review-card__rating">${generateRatingStarsHTML(
                            review.rating
                        )}</div>
                      <div class="review-card__date">${new Date(
                            review.dateObj || review.date
    ...
    function generateRatingStarsHTML(rating) {
        ...
        const assetsPath =
            window.location.pathname.includes("/pages/") || (window.location.pathname.endsWith(".html") && !window.location.pathname.endsWith("index.html")) ? "../assets/" : "assets/";

        for (let i = 0; i < 5; i++) {
            if (i < full)
                stars += `<img src="${assetsPath}star-full.png" alt="★" class="star">`;
            else if (i === full && half)
                stars += `<img src="${assetsPath}star-half.png" alt="½" class="star">`;
            else
                stars += `<img src="${assetsPath}star-empty.png" alt="☆" class="star">`;
        }
