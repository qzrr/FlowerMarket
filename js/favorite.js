...
(function() {
  ...
  async function loadFavoriteItems() {
    if (!favoriteProductsContainer || !emptyFavoriteMessage || !favoriteItemsWrapper) {
      console.warn("Элементы для отображения избранного не найдены на странице.");
      return;
    }
    ...
    const favoriteProductIds = JSON.parse(localStorage.getItem('wishlist')) || []; // Используем 'wishlist' как ключ
    ...
    favoriteProductsContainer.innerHTML = '<div class="loading-spinner">Загрузка избранных товаров...</div>';

    try {
      const productPromises = favoriteProductIds.map(id => API.getProductById(id).catch(err => {
        ...
        const updatedWishlist = currentWishlist.filter(itemId => itemId !== id);
      ...
      if (favoriteProductsData.length === 0) {
        ...
        if (favoriteProductIds.length > 0) { // Если изначально ID были, но не загрузились
      ...
      favoriteProductsContainer.innerHTML = favoriteProductsData.map(window.createProductCardHTML).join('');
      // Навешиваем обработчики на кнопки "в корзину" и "удалить из избранного"
      if (window.Search && typeof window.Search.setupProductCardActionsForContainer === 'function') {
        window.Search.setupProductCardActionsForContainer(favoriteProductsContainer);
      } else {
        console.warn("Функция Search.setupProductCardActionsForContainer не найдена. Кнопки на карточках могут не работать.");
      }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (document.querySelector('.favorite-page')) { // Убедимся, что мы на нужной странице
      loadFavoriteItems();
